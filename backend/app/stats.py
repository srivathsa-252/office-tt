...
def style_tags(pts: list[tuple[Point, str]], player_id: int) -> list[str]:
    ...
    if (rally_won is not None and rally_won <= AGGRESSIVE_MAX_RALLY) or (
    ...
    if rally_all is not None and rally_all >= DEFENSIVE_MIN_RALLY:
    ...
    if len(tagged_lost) >= STYLE_MIN_POINTS and fault_rate <= CONSISTENT_MAX_FAULT_RATE:
    ...
    if (
        serve_win is not None
        and recv_win is not None
        and serve_win - recv_win >= SERVER_RELIANT_MIN_GAP
    ):
        tags.append("server-reliant")
    return tags
