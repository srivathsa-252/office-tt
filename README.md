...
- **Re-enrolling an already-known player adds samples, it doesn't refuse.** Picking an *existing* player by hand used to hit the same "unrecognised faces only" rule as picking a new one — the moment they were recognised well enough, they became ineligible to teach further, so there was no way to add more samples (different lighting, angle, a later day) without first wiping their face data and starting over. `pick_face_for_known_player` (`app/devices/faces.py`) is used instead of `pick_unknown_face` when the enroll request names a player: a face already recognised as *them* is accepted as another sample; a face recognised as someone else is refused outright, so it can't get mixed into the wrong gallery.
- **Calibrating the match threshold to your own gallery.** `python -m app.devices.calibrate_threshold` compares every pair of samples belonging to the same enrolled player against every pair belonging to different players, and reports whether `MATCH_THRESHOLD` (0.363, SFace's generic published value) actually separates them cleanly for the people you've enrolled — no retraining, just statistics on samples already collected. If the two overlap, it says so and suggests enrolling more/varied samples instead of just moving the number.
...
**Frame guide, not full frame.** The scan wizard's preview overlays a small centred oval (42% of the preview's width, portrait 3:4) and dims everything outside it, with "Come into frame — fill the oval with your face" shown until a scan lands. This is enforced on the worker, not just drawn in the browser: `in_guide_region()` (`app/devices/faces.py`) mirrors the same fraction against the real analysed frame, and `enroll_step` filters to only faces centred there before either accepting a sample or checking "already known" — a face elsewhere in a wide shot (someone in the background, off to the side) is ignored rather than silently analysed. Initially the oval was cosmetic only (the backend still scanned the whole frame), which let an off-centre, poor-angle face get picked up and sometimes misidentified. The oval also rendered visibly off-centre itself (shifted toward the bottom-right) — its `translate(-50%,-50%)` centering and its `tt-scan-pulse` breathing animation lived on the same element, and an animation's `transform` keyframes replace the element's whole transform for as long as it runs, silently dropping the centering the instant the pulse started. Fixed by splitting the two onto nested elements: an outer div does the exact `left:50%; top:50%; translate(-50%,-50%)` centering with no animation on it, an inner div fills it and owns the pulse.

**Not-quite-right matches.** `already_known_match` — the "are you already **&lt;name&gt;**?" check — used to trust any single face in view that crossed the bare 0.363 similarity threshold, with no size or confidence floor, unlike enrolling a new face which already required `ENROLL_MIN_SCORE` (0.9) and `ENROLL_MIN_SIZE_PX` (80px). A small or poor-angle face could cross 0.363 by noise alone and get matched to the wrong existing player. It's now held to the same quality bar as enrolling: a face too small or too low-confidence to safely learn is also too unreliable to claim it's someone specific.
